...
def tsx_losers():
    ...
    # Nyse loser
    results = requests.get('https://www.investcom.com/us/mplnyse.htm', verify=False)
    soup = bs4.BeautifulSoup(results.text, "xml")
    div1 = soup.find_all('div', {"class": "genTable"})
    table = div1[0].find('table')
    u = table.find_all('u')
    ...
    if results.status_code == 200:
        ...
        print("Nyse Losers ")
        for key, value in res.items():
            Nyse_losers.objects.create(symbol=key, name=value[0], change=value[1], percent=value[2])

    print(Nyse_losers.objects.all(),"nyse nnnnnlosersssssss from db")

    # nyse gainers
    results = requests.get('https://www.investcom.com/us/mpgnyse.htm', verify=False)
    soup = bs4.BeautifulSoup(results.text, "xml")
    div1 = soup.find_all('div', {"class": "genTable"})
    table = div1[0].find('table')
    u = table.find_all('u')
    ...
    # if len(td) > 24:
    #     res[td[24].text.lstrip()] = [td[27].text, td[26].text, td[25].text[:-7]]
    print("nyse_newwwwwwgianerss", results.status_code, str(datetime.now()))
    if results.status_code == 200:
        Nyse_gainers.objects.all().delete()
        print("Nyse gainers ")
        for key, value in res.items():
            Nyse_gainers.objects.create(symbol=key, name=value[0], change=value[1], percent=value[2])


    # nasdaq losers
    results = requests.get('https://www.investcom.com/us/mplnasdaq.htm', verify=False)
    soup = bs4.BeautifulSoup(results.text, "xml")
    div1 = soup.find_all('div', {"class": "genTable"})
    table = div1[0].find('table')
    u = table.find_all('u')
    ...
    # if len(td) > 24:
    #     res[td[24].text.lstrip()] = [td[27].text, td[26].text, td[25].text[:-7]]
    results = requests.get('https://www.investcom.com/us/mpgnasdaq.htm', verify=False)
    soup = bs4.BeautifulSoup(results.text, "xml")
    div1 = soup.find_all('div', {"class": "genTable"})
    table = div1[0].find('table')
    u = table.find_all('u')
